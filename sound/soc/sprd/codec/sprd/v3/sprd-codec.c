...
extern void headsetmic_bypass(void);
...
struct sprd_codec_priv {
	...
	struct delayed_work speaker_delayed_work;
...
static inline void sprd_codec_inter_pa_init(struct sprd_codec_priv *sprd_codec)
{
	...
	sprd_codec->inter_pa.setting.befor_pa_delay_50ms = 2;
	sprd_codec->fSpkDlySetting = false;
}
...
#define to_sprd_codec_spk(x) container_of((x), struct sprd_codec_priv, speaker_delayed_work)
static void sprd_inter_speaker_pa_delay_worker(struct work_struct *work)
{
	struct sprd_codec_priv *sprd_codec =
	    to_sprd_codec_spk(to_delay_worker(work));
	struct snd_soc_codec *codec = sprd_codec->codec;
	struct sprd_codec_inter_pa *p_setting = &sprd_codec->inter_pa.setting;

	if(!sprd_codec->fSpkDlySetting) {
		sp_asoc_pr_info("SPK Delay work Skipped : SPK closed already!!");
		return;
	}

	mutex_lock(&sprd_codec->inter_pa_mutex);
	sp_asoc_pr_info("SPK Delay work START!!!!!\n");
...
static int sprd_inter_speaker_pa(struct snd_soc_codec *codec, int on)
{
	...
	if (on) {
		sprd_codec->fSpkDlySetting = true;
		sp_asoc_pr_info("sprd_inter_speaker_pa ctrl START!!!!!!!\n");
		schedule_delayed_work(&sprd_codec->speaker_delayed_work,
					       ...
					       *sprd_codec)
{
	...
	sprd_codec->inter_hp_pa.setting.class_g_unmute_delay_100ms = 4;
			   ...
			   struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		/* enable the diff function in shark from weifeng.ni */
		...
		if (sprd_codec->hp_ver == SPRD_CODEC_HP_PA_VER_1) { /*2713, 2713s*/
			snd_soc_update_bits(codec, SOC_REG(AUD_SDM_CTL0), 0xfff,
				0x400);
		} else if (sprd_codec->hp_ver == SPRD_CODEC_HP_PA_VER_2) {/*2711*/
			snd_soc_update_bits(codec, SOC_REG(AUD_SDM_CTL0), 0xfff,
				0x500);
		}
#ifndef CONFIG_SND_SOC_SPRD_CODEC_NO_HP_POP
			  ...
			  struct snd_ctl_elem_value *ucontrol, bool need_set)
{
	...
	if (mixer->set && (need_set || !mixer->on))
...
static int sprd_codec_hpmic_bypass_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	return 0;
...
static int sprd_codec_hpmic_bypass_put(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	headsetmic_bypass();
	return 0;
...
static int sprd_codec_hp_internal_mute_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int sprd_codec_hp_internal_mute_put(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
...
static const struct snd_kcontrol_new sprd_codec_snd_controls[] = {
	...
	SOC_SINGLE_EXT("Headmic Bypass", 0, 0, LONG_MAX, 0,
		       sprd_codec_hpmic_bypass_get, sprd_codec_hpmic_bypass_put),

	SOC_SINGLE_EXT("Headphone Internal Mute", 0, 0, LONG_MAX, 0,
		       sprd_codec_hp_internal_mute_get, sprd_codec_hp_internal_mute_put),
...
static int sprd_codec_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&sprd_codec->speaker_delayed_work,
			  sprd_inter_speaker_pa_delay_worker);
