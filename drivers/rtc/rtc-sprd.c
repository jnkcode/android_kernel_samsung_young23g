...
#define ANA_RTC_SPG_VALUE				(RTC_BASE + 0x50)	//add by wbl
...
//static bool rtc_reset_flag = false;	//add by wbl
...
static int __init sprd_rtc_init(void)
{
	...
	printk("After reset RTC time: %d-%d-%d %d:%d:%d\n",read_tm.tm_year+1900,read_tm.tm_mon+1,read_tm.tm_mday,read_tm.tm_hour,read_tm.tm_min,read_tm.tm_sec);
